...
7. **这不是安全边界，是漂移护栏（如实标注，2026-09-11）**：任何 host 侧插件代码都能对**任意会话** append `sandbox/mode` —— 本插件自己就是这么绑定的（ADR-014 的设计），§13 那次救场也是这么做的。本次为完成"装 0.8.6 进 profile + `git push`"这两步**只能在区外做**的操作，同样用这一机制把**本会话**临时切到 `danger-full-access`，做完立刻切回 `workspace-write`（切回后实测：区内写 OK、区外写与 profile 写**均被拒**、端口枚举又回到失真 —— 边界完好）。
   结论要写清楚：**它挡的是"无意的漂移"（agent 顺手写到别处），不是"有意的绕过"**。对后者的约束在制度层——人是授权者、策略行与会话日志留痕、每次放宽都可审计——而不在沙箱。把这条当作安全控制来依赖是错的。

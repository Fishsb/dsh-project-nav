# [项目事实] reference · project-nav 已知代码问题（v0.7.3 复核）

- 卡类型：reference
- 溯源：project-nav 审查 seq37326（原始快照已过时；本卡按 **v0.7.3** 实测逐项复核）
...
| `package.json` scripts 指向缺失文件 | ✅ 已修（v0.4.0） | `test = node --test test/core.test.mjs test/concurrency.test.mjs`，**54 用例实跑全绿** |
...
| **验证动作被算成「补丁」（假计数）** | ✅ 已修（v0.7.3） | 计数闸只数**有指纹证据证明真的动过 scope 文件**的动作；纯验证/记账动作记 `noChange` 不计入，无证据者照旧计入（保守优先）。否则会逼出空心 ADR、使核心决策账本贬值。实机证据：ACT-006（done 时明确报「none of the scoped files changed」）曾把 PN-F01 推到 2/3 |
| 部署声明与实体不一致 | ✅ 当前一致（v0.7.3） | 声明 `file:…dsh-external-project-nav-0.7.3.tgz` = 产物内部 0.7.3 = 已装副本 0.7.3（四者 SHA256 MATCH）。**注意这是每版必做的动作**，见下「流程风险」 |

> 主题贯穿：v0.7.1 / v0.7.2 / v0.7.3 修的是**同一件事**——假信号（假 STALE / 假「索引外文件」/ 假补丁计数）腐蚀信任，而信号可靠性是防漂移插件的全部依据。新增任何闸门或提示时，先问「这个信号的误报率是多少」。

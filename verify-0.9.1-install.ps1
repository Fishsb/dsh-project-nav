# verify-0.9.1-install.ps1 — 独立校验 0.9.1 是否真的装上（**只读**，可在重启前跑）
#
# 为什么需要它：本仓历史上有"接口成功≠达成"的教训（假绿），
# 装插件至少要证四件事：产物指纹、声明、实体、装配副本内容。
# 任一条不过就红字退出 1。
#
# 用法：powershell -NoProfile -ExecutionPolicy Bypass -File verify-0.9.1-install.ps1
#       ... -TgzSha256 <打包时记下的哈希>   ... -ProfileDir <别的 profile 路径>

param(
  [string]$TgzSha256 = 'ED9B714A7343A62EB8912F77EDBC47AA2E673757EE7D7AE8F32F5F6ACF01457B',   # 打包时算的 SHA256
...
Write-Host '=== project-nav 0.9.1 安装校验（只读） ==='
...
if (Test-Path $ProfilePkg) {
  ...
  Check '声明指向 0.9.1' ($decl -like '*0.9.1*') "dependencies = $decl"
...
if (Test-Path (Join-Path $Installed 'package.json')) {
  ...
  Check '实体版本 = 0.9.1' ($ip.version -eq '0.9.1') "已装版本 = $($ip.version)"
...
Write-Host ''
if ($ok) {
  Write-Host '结论：安装面全绿。剩余一步是**重启 dsh-web**（禁 agent 自行重启），重启后新会话才会加载 0.9.1。' -ForegroundColor Green

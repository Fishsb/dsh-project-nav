# verify-0.9.2-install.ps1 — 独立校验 0.9.2 是否真的装上（**只读**，可在重启前跑）
#
# 为什么需要它：本仓历史上有"接口成功≠达成"的教训（假绿），
# 装插件至少要证四件事：产物指纹、声明、实体、装配副本内容。
# 任一条不过就红字退出 1。
#
# 用法：powershell -NoProfile -ExecutionPolicy Bypass -File verify-0.9.2-install.ps1
#       ... -TgzSha256 <打包时记下的哈希>   ... -ProfileDir <别的 profile 路径>

param(
  [string]$TgzSha256 = '0AF54BFF96EE9AFAF1E390E465722D65D1864A1FD015372DFBA1C8D0D5C9F06C',   # 打包时算的 SHA256
...
$Tgz        = 'D:\FF\project-nav\dsh-external-project-nav-0.9.2.tgz'
...
Write-Host '=== project-nav 0.9.2 安装校验（只读） ==='
...
if (Test-Path $ProfilePkg) {
  ...
  Check '声明指向 0.9.2' ($decl -like '*0.9.2*') "dependencies = $decl"
...
# 3) 安装实体身份（0.9.x 的特征：有 core/、没有 shared/）
if (Test-Path (Join-Path $Installed 'package.json')) {
  ...
  Check '实体版本 = 0.9.2' ($ip.version -eq '0.9.2') "已装版本 = $($ip.version)"
  Check '实体含 core/' (Test-Path (Join-Path $Installed 'core')) 'core/ 目录'
  Check '实体已无 shared/' (-not (Test-Path (Join-Path $Installed 'shared'))) 'shared/ 目录应已消失（0.9.x 用 core/）'
  ...
  Check '实体与仓库源码逐字节一致（全部 core/host）' ($diff.Count -eq 0) $(if ($diff.Count) { "不一致 $($diff.Count) 个: $($diff -join ', ') —— 说明装的是旧一版，需重新安装" } else { "全部 $($pairs.Count) 个文件一致" })
} else { Check '安装实体存在' $false $Installed }

Write-Host ''
if ($ok) {
  Write-Host '结论：安装面全绿。剩余一步是**重启 dsh-web**（禁 agent 自行重启），重启后新会话才会加载 0.9.2。' -ForegroundColor Green

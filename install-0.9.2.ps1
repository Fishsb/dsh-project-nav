# install-0.9.2.ps1 — 把 project-nav 0.9.2 装进 profile（**需你在自己终端执行**）
#
...
#
# 用法：powershell -NoProfile -ExecutionPolicy Bypass -File D:\FF\project-nav\install-0.9.2.ps1
...
} else { Write-Host '[5/5] 声明已是 0.9.2' }
...
if ($ip.version -ne '0.9.2') { Fail "实体版本 = $($ip.version)，不是 0.9.2" }
...
Write-Host '  powershell -NoProfile -ExecutionPolicy Bypass -File D:\FF\project-nav\verify-0.9.2-install.ps1'
Write-Host ''

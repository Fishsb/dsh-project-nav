...
> F6/F7 是**宿主侧**约束：0.9.2 拆掉工作区边界插件化后，这两条不再由本插件满足（改由宿主原生权限面承担），但事实本身仍不可丢。
...
| F6 | Windows `workspace-write` 只有一个可写根、不含 `~/.dsh` | **0.9.2 起不在本插件内满足**：插件不设/不切/不探沙箱，模式与审批归宿主（`dsh-sandbox-policy` + `dsh-permission-presets` 三档预设 + UI 切换）。该事实作为宿主约束保留 |
| F7 | 服务账户无 logon SID ⇒ 沙箱后端起不来 | 同上：宿主侧约束（v0.8.x 的探针/免绑策略随边界机制一并拆除） |
